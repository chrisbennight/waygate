...
| `CatalogDrift` | Emitted for observed tool-contract changes during refresh, reconnect, and annotation-mode dispatch checks. Manifest mode hashes name, description, and input schema; annotation mode also covers output schema and security metadata. With durable review configured, the persisted baseline detects changes across restart. Initial baseline creation is silent. Drift evidence uses chained best effort, with `Denied` for quarantine and `Success` for informational changes; stored review decisions remain authoritative if evidence delivery fails. |
| `DataInspection` | Reserved; the built-in PII, secrets, and poisoning inspectors record findings on the dispatched call's `Invocation` audit row. Stored custom inspection rules are not enforced. No separate per-finding event is emitted. |
